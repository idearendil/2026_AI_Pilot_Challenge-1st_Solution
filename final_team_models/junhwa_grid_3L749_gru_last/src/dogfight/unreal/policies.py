...
class ProviderCommandPolicy:
    def __init__(
        ...
        required_repeat = getattr(self.action_provider, "required_action_repeat", None)
        if required_repeat is not None and self.action_repeat != required_repeat:
            raise ValueError(f"Provider observation contract requires action_repeat={required_repeat}")
    ...
    def _compute_provider_action(self, context, own_plane, enemy_plane) -> np.ndarray:
        ...
        observation = (None if getattr(self.action_provider, "builds_own_observation", False)
                       else self._build_observation(ownship_state, target_state))
