...
CPU 학습 번들은 기존 context 관측 경로를 보존한다. CUDA 학습 번들은
버전이 명시된 전용 reconstructor로 관측과 command-space 행동 이력을 만든다.
프로토콜 인터페이스 일치만으로 학습/제출 관측이 일치한다고 가정하지 않는다.
"""
...
from claude_code.observation_contract import CUDAObservationState, uses_cuda_observation_contract
...
class MLPActionProvider(ActionProvider):
    def __init__(self, bundle_dir: str | Path, device: str = "cpu", confidence: float = 0.9,
        ...
        self._cuda_observation = (CUDAObservationState()
                                 if uses_cuda_observation_contract(self.metadata) else None)
        self.builds_own_observation = self._cuda_observation is not None
        self.required_action_repeat = 6 if self._cuda_observation is not None else None
        # 학습 때와 동일한 관측 정규화 적용 (없으면 항등).
        self._normalize_obs = make_obs_normalizer(self.metadata.get("obs_normalization"))
        self._actor_state = (self.model.initial_state(1, self.device)
        ...
        self._reconstruct = (self.metadata.get("observation_module") == "claude_code.my_observation"
                             and self._cuda_observation is None)
    ...
    def reset(self, context: ActionContext | None = None) -> None:
        if self._cuda_observation is not None:
            self._cuda_observation.reset()
    ...
    def _prepare_observation(self, context: ActionContext):
        if self._cuda_observation is not None:
            if context.ownship_state is None or context.target_state is None:
                raise ValueError("CUDA bundle requires both states to reconstruct its observation")
            return self._cuda_observation.observation(context.ownship_state, context.target_state)
        # Preserve the historical CPU-trained bundle convention. CUDA bundles
        # use the private post-step reconstruction above instead of this path.
    ...
    def compute_action(self, context: ActionContext) -> ActionResult:
        ...
        if not np.isfinite(obs).all():
            raise FloatingPointError("Non-finite normalized policy observation")
        obs_tensor = torch.as_tensor(obs, dtype=torch.float32, device=self.device).unsqueeze(0)
        if getattr(self.model, "is_recurrent", False):
            with torch.no_grad():
                raw_t, _, _, self._actor_state = self.model.actor_step(
                    obs_tensor, self._actor_state, deterministic=not self.stochastic)
                self._actor_state = tuple(x.detach() for x in self._actor_state)
        ...
        if self._cuda_observation is not None:
            self._cuda_observation.record_command(command)
