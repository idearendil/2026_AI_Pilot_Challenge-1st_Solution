...
각속도와 LOS rate 재구성
------------------------
ownship과 target p/q/r은 StateReconstructor.advance()에서 직전 자세와 현재 자세의
SO(3) log로 추정하므로 yaw가 179 deg -> -179 deg로 wrap되어도 인위적인 spike가
생기지 않는다. target p/q/r과 LOS azimuth/elevation rate 다섯 항을 기존 42차원 뒤에
추가하며, 기존 0~41번 feature의 의미와 순서는 바꾸지 않는다.
"""
...
PQR_SCALE_RAD_S = 4.0
LOS_RATE_SCALE_RAD_S = 2.0
LOS_AZIMUTH_MIN_HORIZONTAL_FRACTION = 1.0e-3
AOA_SCALE_DEG = 30.0
...
OBSERVATION_MODE = "claude47r"
OBSERVATION_SIZE = 47

# ── 47-D observation feature 요약 (build_observation 참고) ────────────────────
...
#   42   target_p_est_tanh           표적 롤레이트 SO(3) log 추정 tanh(/4)
#   43   target_q_est_tanh           표적 피치레이트 〃
#   44   target_r_est_tanh           표적 요레이트 〃
#   45   los_azimuth_rate_tanh       LOS 방위각 변화율 tanh(/2 rad/s)
#   46   los_elevation_rate_tanh     LOS 고각 변화율 tanh(/2 rad/s)
#
...
def _los_azimuth_is_well_conditioned(own_state, target_state) -> bool:
    """Suppress azimuth derivatives when LOS is almost body-vertical."""
    ...
    if distance <= 1.0e-9:
        return False
    relative_body = _ned_to_body_matrix(
        own[StateIndex.ROLL],
        own[StateIndex.PITCH],
        own[StateIndex.YAW],
    ) @ relative_ned
    horizontal = float(np.hypot(relative_body[0], relative_body[1]))
    return horizontal / distance >= LOS_AZIMUTH_MIN_HORIZONTAL_FRACTION
...
class StateReconstructor:
    ...
    def reset(self) -> None:
        ...
        self.prev_target_att = None
        self.target_pqr_est = np.zeros(3, dtype=np.float64)
        self.prev_los_angles = None
        self.prev_los_azimuth_valid = False
        self.los_rates = np.zeros(2, dtype=np.float64)
    ...
    def advance(self, own_state, tgt_state) -> None:
        ...
        target_att = np.array([
            tgt[StateIndex.ROLL],
            tgt[StateIndex.PITCH],
            tgt[StateIndex.YAW],
        ], dtype=np.float64)
        if self.prev_target_att is None:
            self.target_pqr_est[:] = 0.0
        else:
            r_prev_target = _body_to_ned_matrix(
                self.prev_target_att[0],
                self.prev_target_att[1],
                self.prev_target_att[2],
            )
            r_curr_target = _body_to_ned_matrix(
                target_att[0],
                target_att[1],
                target_att[2],
            )
            target_rotvec = _log_so3(r_prev_target.T @ r_curr_target)
            self.target_pqr_est = target_rotvec / max(self.dt, 1e-8)
        self.prev_target_att = target_att

        azimuth, elevation = self._geo._get_los_angle(own, tgt)
        current_los = np.array([azimuth, elevation], dtype=np.float64)
        current_azimuth_valid = _los_azimuth_is_well_conditioned(own, tgt)
        if self.prev_los_angles is None:
            self.los_rates[:] = 0.0
        else:
            azimuth_delta_deg = 0.0
            if current_azimuth_valid and self.prev_los_azimuth_valid:
                azimuth_delta_deg = _wrapped_delta_deg(
                    current_los[0], self.prev_los_angles[0]
                )
            elevation_delta_deg = _wrapped_delta_deg(
                current_los[1], self.prev_los_angles[1]
            )
            self.los_rates = np.deg2rad(
                [azimuth_delta_deg, elevation_delta_deg]
            ) / max(self.dt, 1e-8)
        self.prev_los_angles = current_los
        self.prev_los_azimuth_valid = current_azimuth_valid
...
def build_observation(ownship_state, target_state, geo_info, wez_config=None,
    ...
    """47차원 claude47r observation을 만든다.
    ...
    # 42-44: target body angular rates reconstructed causally with SO(3).
    target_pqr = np.asarray(rec.target_pqr_est, dtype=np.float64)
    obs[42] = _tanh_scale(float(target_pqr[0]), PQR_SCALE_RAD_S)
    obs[43] = _tanh_scale(float(target_pqr[1]), PQR_SCALE_RAD_S)
    obs[44] = _tanh_scale(float(target_pqr[2]), PQR_SCALE_RAD_S)

    # 45-46: ownship-body LOS azimuth/elevation rates.  The azimuth rate is
    # suppressed close to the vertical singularity by StateReconstructor.
    obs[45] = _tanh_scale(float(rec.los_rates[0]), LOS_RATE_SCALE_RAD_S)
    obs[46] = _tanh_scale(float(rec.los_rates[1]), LOS_RATE_SCALE_RAD_S)
...
def describe_observation() -> dict:
    return {
        ...
        "features": [
            ...
            "target_q_est_tanh",
            "target_r_est_tanh",
            "los_azimuth_rate_tanh",
            "los_elevation_rate_tanh",
        ],
        "description": (
            "claude47r 47-D observation. 기존 claude42r의 0~41번 feature를 "
            "그대로 유지하고 target p/q/r 및 LOS azimuth/elevation rate를 추가한다. "
            "현재 state[6:9]는 body-frame 속도 "
            "u/v/w로 처리한다. speed + attitude로 NED 속도를 재구성하지 않는다. "
            "ownship p/q/r은 StateReconstructor에서 SO(3) log로 추정한다. "
            "target p/q/r도 같은 SO(3) log로 추정한다. 상대 위치는 ownship body frame에서 "
