...
from .model import policy_action_to_command, discrete_indices_to_continuous
from .my_observation import StateReconstructor
...
class SelfPlayProvider(ActionProvider):
    ...
    def compute_action(self, context: ActionContext) -> ActionResult:
        if self._cached is None or self._count % self.repeat == 0:
            ...
            with torch.no_grad():
                if self.explore:
                    # Opponent control needs only an actor sample. Computing the
                    # critic here doubled the MLP work without affecting action
                    # probabilities or RNG consumption.
