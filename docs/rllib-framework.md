# DogFight RL 학습 환경 매뉴얼 (RLlib 프레임워크 · 레거시)

> **참고**: 이 문서는 원래 저장소 루트의 `README.md` 였습니다. 현재 팀의 주 학습 경로는
> `claude_code/` 순수 PyTorch PPO 패키지로 옮겨졌고(루트 `README.md` 참고), 이 문서는
> RLlib 기반 학습 프레임워크(`train_rllib.py`, `train_curriculum.py`, `experiments/*.yaml`,
> `student/`)를 쓸 때를 위해 보존한 레거시 매뉴얼입니다. 해당 코드/자산은 그대로 남아 있습니다.
