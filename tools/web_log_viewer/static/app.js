...
// f16_simple_cc_by.obj 는 +x 가 기수다(캐노피 x≈0.15~2.2, 수직미익 x=-5.4, 노즈콘 x=8.0).
// 예전엔 180 이라 기체가 뒤를 보고 날았다. attitudeMatrix 의 pitch 부호 버그와 겹쳐
// 실제 속도 벡터와 118도나 어긋나 있었다(수정 후 평균 7.9도 = 받음각 수준).
...
function attitudeMatrix(rollDeg, pitchDeg, yawDeg) {
  ...
  // 표시 좌표계가 z-up 이라 +y 축 회전은 기수를 내린다. pitch(+)=기수 상승이므로 -pitch.
  // (log_data.attitude_matrix 와 동일해야 함)
