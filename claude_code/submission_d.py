"""[claude_code] neural-MPC 제출 — submission_a/b 와 같은 번들 + neural MPC (인스턴스 D).

submission_b.py 와 **같은 모델 번들**(BUNDLE_DIR)을 쓰되, 단일 MLP 정책 대신
`MPCActionProvider` 로 obs-WM 1~2초 lookahead 를 굴려 first-action 을 고른다.
submission_c.py 와 TEAM_NAME 만 다르다(로컬 self-play 테스트용 페어).
...
TEAM_NAME = "team02"                                   # 인스턴스 D 팀 이름 (C 와 달라야 함)
...
def main():
    print(f"=== [claude_code] {TEAM_NAME} neural-MPC 제출 시작 (인스턴스 D) ===")
