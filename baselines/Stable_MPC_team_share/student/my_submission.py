...
from safe_mpc.command_policy import SafeMPCCommandPolicy  # noqa: E402
from safe_mpc.config import load_safe_mpc_config  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--safety-config",
        default=str(ROOT / "configs" / "safe_mpc.yaml"),
    )
...
def main() -> None:
    args = parse_args()
    mpc_config = load_config(args.mpc_config)
    safety_config = load_safe_mpc_config(args.safety_config)
    policy = SafeMPCCommandPolicy(ROOT, mpc_config, safety_config)
